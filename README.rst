...
You can determine which is the default interpreter by running the command ``which python`` (for Mac) or ``where python`` (for Windows) from a command prompt window. If the path that these commands output contains ``python36``, you should be fine -- if it contains ``python27`` instead, you will need to be careful. If this is the case, you can ensure that you are always using the correct interpreter by affixing a 3 to commands involving Python -- for example, use ``pip3`` instead of ``pip`` and ``python3`` instead of ``python``.
